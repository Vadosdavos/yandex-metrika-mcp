...
server.registerTool("get_account_info",
  {
    ...
    inputSchema: { counter_id: z.string().describe("Yandex Metrika counter ID") }
  },
  async ({ counter_id }) => {
    try {
      const accountInfo = await client.getAccountInfo(counter_id);
    return {
      content: [{ type: "text", text: `Account info: ${JSON.stringify(accountInfo)}` }]
    }
    } catch (error) {
      return {
        content: [{ type: "text", text: `Something went wrong. Error: ${error}` }]
      }
    }
  }
);
...
server.registerTool("get_visits",
  {
    ...
    inputSchema: { counter_id: z.string().describe("Yandex Metrika counter ID"), date_from: z.string().optional().describe("Date from"), date_to: z.string().optional().describe("Date to") }
  },
